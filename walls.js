  ...
  createListingOnBackend: function(listing) {
    ...
    return window._wwApi(cfg.LISTINGS || '/api/listings', { method: 'POST', body: listing, timeout: 60000 })
      .then(res => {
        ...
        // Preserve the client-side id so the rest of the UI keeps working
        // even if the backend assigned a different _id.
        if (!saved.id) saved.id = saved.clientId || listing.id;
        return saved;
      })
      .catch(err => {
        ...
        if (typeof showToast === 'function') {
          showToast('Saved locally — backend sync failed: ' + msg, 'warning');
        }
        return listing;
      });
  },

...
self.addEventListener('fetch',e=>{
  ...
  loadListingsFromStorage: function() {
    ...
    window._wwApi(cfg.LISTINGS || '/api/listings', { method: 'GET', timeout: 45000, noAuth: true })
      .then(function(data) {
  ...
  saveListingsToStorage: function() {
    ...
    // Single-record creates/updates/deletes already hit dedicated REST endpoints
    // (createListingOnBackend, updateListingOnBackend, deleteListingOnBackend),
    // so no destructive bulk PUT here — that was overwriting other devices' data.
  },
